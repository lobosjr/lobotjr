...
namespace TwitchBot
{
    ...
    class Program
    {
        ...
        static void HandleCommandResult(string whisperSender, string whisperMessage, CommandResult result, IrcClient irc, TwitchClient twitchClient)
        {
            ...
            {
                Logger.Error("Errors encountered while processing command \"{0}\" from user {1}", whisperMessage, whisperSender);
            ...
            {
                Logger.Debug("Debug output generated by command \"{0}\" from user {1}", whisperMessage, whisperSender);
        ...
        static void UpdateDungeons(string dungeonListPath, ref Dictionary<int, string> dungeonList)
        {
            ...
            else
            {
                ...
                Logger.Error("Failed to load dungeon list file, {0} not found.", dungeonListPath);
            }
            ...
            foreach (var line in fileText)
            {
                ...
                if (id != -1)
                    ...
                    Logger.Error("Invalid dungeon read on line {0}", dungeonIter);
                dungeonIter++;
            }
        }
        ...
        static void UpdateItems(string itemListPath, ref Dictionary<int, string> itemList, ref Dictionary<int, Item> itemDatabase)
        {
            ...
            {
                fileText = new List<string>();
                Logger.Error("Failed to load item list file, {0} not found.", itemListPath);
            }
            ...
            foreach (var line in fileText)
            {
                ...
                if (id != -1)
                    ...
                    Logger.Error("Invalid item read on line {0}", itemIter);
                itemIter++;
            }
        ...
        static void UpdatePets(string petListPath, ref Dictionary<int, string> petList, ref Dictionary<int, Pet> petDatabase)
        {
            ...
            {
                fileText = new List<string>();
                Logger.Error("Failed to load item list file, {0} not found.", petListPath);
            }
            ...
            foreach (var line in fileText)
            {
                ...
                if (id != -1)
                    ...
                    Logger.Error("Invalid pet read on line {0}", petIter);
                petIter++;
            }
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                else
                {
                    ...
                    Logger.Warn("Failed to load subathon file, {0} not found.", subathonPath);
                }
                ...
                {
                    Logger.Info($"Database is out of date, updating to {0}. This could take a few minutes.", SqliteDatabaseUpdater.LatestVersion);
                    ...
                    while (true)
                    {
                        ...
                        if (userLookup.IsUpdateTime(DateTime.Now))
                        {
                            ...
                            foreach (var user in cacheUpdateResults.FailedUsers)
                            {
                                ...
                                Logger.Warn("Failed to lookup id for user {0}. It's possible the username we're getting from IRC doesn't match, maybe due to special characters or something? Not sure...", user);
                            }
                        }
                                ...
                                else if (whisperMessage.StartsWith("!bug"))
                                {
                                    if (wolfcoins.Exists(wolfcoins.classList, whisperSender))
                                    {
                                        ...
                                        if (msgData.Count() > 1)
                                        {
                                            ...
                                            Logger.Info("{0} submitted a bug report.", whisperSender);
                                        }
                                    }
                                }
                                ...
                                else if (whisperMessage == "y")
                                {
                                    if (wolfcoins.Exists(wolfcoins.classList, whisperSender))
                                    {
                                        ...
                                        if (wolfcoins.classList[whisperSender].pendingInvite)
                                        {
                                            ...
                                            Logger.Info("{0} added to Group {1}", whisperSender, partyID);
                                        ...
                                        if (myID != -1 && !wolfcoins.classList[whisperSender].pendingInvite)
                                        {
                                            if (parties.Count() > 0 && parties.ContainsKey(myID))
                                            {
                                                if (wolfcoins.classList[whisperSender].isPartyLeader)
                                                {
                                                    ...
                                                    Logger.Info("Party Leader {0} left group #{1}", whisperSender, myID);
                                                    string myMembers = "";
                                                    foreach (var member in parties[myID].members)
                                                    {
                                                        myMembers += member.name + " ";
                                                    }
                                                    Logger.Info("Remaining members: {0}", myMembers);
                                                ...
                                                else if (parties.ContainsKey(myID) && (parties[myID].RemoveMember(whisperSender)))
                                                {
                                                    ...
                                                    wolfcoins.classList[whisperSender].groupID = -1;
                                                    wolfcoins.classList[whisperSender].ClearQueue();
                                                    Logger.Info("{0} left group with ID {1}", whisperSender, myID);
                                                    string myMembers = "";
                                                    foreach (var member in parties[myID].members)
                                                    {
                                                        myMembers += member.name + " ";
                                ...
                                else if (whisperMessage == "!createparty")
                                {
                                    if (wolfcoins.Exists(wolfcoins.classList, whisperSender))
                                    {
                                        if (!wolfcoins.classList[whisperSender].pendingInvite && wolfcoins.classList[whisperSender].groupID == -1)
                                        {
                                            ...
                                            Logger.Info("ID: {0}", maxPartyID);
                                            Logger.Info("Total number of parties: {0}", parties.Count());
                                            maxPartyID++;
                                        }
                                        else if (wolfcoins.classList[whisperSender].isPartyLeader)
                                        {
                                ...
                                else if (whisperMessage.StartsWith("!printinfo"))
                                {
                                    ...
                                    if (whispers.Length >= 2 && whispers[1] != null)
                                    {
                                        ...
                                        if (wolfcoins.Exists(wolfcoins.classList, player))
                                        {
                                            ...
                                            Logger.Info("Name: {0}", player);
                                            Logger.Info("Level: {0}", wolfcoins.classList[player].level);
                                            Logger.Info("Prestige Level: {0}", wolfcoins.classList[player].prestige);
                                            Logger.Info("Class: {0}", wolfcoins.classList[player].className);
                                            Logger.Info("Dungeon success chance: {0}", wolfcoins.classList[player].GetTotalSuccessChance());
                                            Logger.Info("Number of Items: {0}", numItems);
                                            Logger.Info(wolfcoins.classList[player].PrintItems());

                                        }
                                        else
                                        {
                                            Logger.Info("Player name \"{0}\" not found.", player);
                                        }
                                    ...
                                    if (wolfcoins.xpList != null)
                                    {
                                        ...
                                        foreach (var viewer in wolfcoins.xpList)
                                        {
                                            ...
                                            if (myLevel >= 3 && !wolfcoins.classList.ContainsKey(viewer.Key))
                                            {
                                                ...
                                                Logger.Info("Added {0} to the Class List.", viewer.Key);
                                            }
                                        }
                                        wolfcoins.SaveClassData();
                                    }
                            ...
                            if (message[0] != null && message[1] != null)
                            {
                                ...
                                if (sender == "twitchnotify" && first.Last() == "subscribed!")
                                {
                                    ...
                                    if (!wolfcoins.subSet.Contains(newSub))
                                    {
                                        ...
                                        Logger.Info("Added {0} to the subs list.", first[0]);
                                    }
                                }
                                    ...
                                    case "!removecoins":
                                        {
                                            ...
                                            if (sender == tokenData.BroadcastUser || sender == tokenData.ChatUser)
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    Logger.Warn("Sender: {0}", sender);
                                                    Logger.Warn("Target: {0}", target);
                                                    Logger.Warn("Amount: {0} coins.", coins);
                                                }
                                            }
                                            else
                                            {
                                                Logger.Warn("Non-Moderator \"{0}\" attempted to remove coins.", sender);
                                    ...
                                    case "!addcoins":
                                        {
                                            ...
                                            {
                                                Logger.Warn("Non-Moderator \"{0}\" attempted to add coins.", sender);
                                                irc.sendChatMessage("Sorry, " + sender + ", AddCoins is a moderator-only command.");
                                            }
            ...
            catch (Exception ex)
            {
                ...
                Logger.Error("The application has encountered an unexpected error: {0}", ex.Message);
                Logger.Error(ex);
                Logger.Error("The full details of the error can be found in {0}", file);
        ...
        static Pet GrantPet(string playerName, Currency wolfcoins, Dictionary<int, Pet> petDatabase, IrcClient irc, TwitchClient twitchClient)
        {
            ...
            if (toAward.Count > 0)
            {
                ...
                if (newPet.isSparkly)
                {
                    Logger.Info("WOW! {0} just found a SPARKLY pet {1}", playerName, newPet.name);
                    irc.sendChatMessage("WOW! " + playerName + " just found a SPARKLY pet " + newPet.name + "! What luck!");
                }
                else
                {
                    Logger.Info("{0} just found a pet {1}!", playerName, newPet.name);
        ...
        static int GrantItem(int id, Currency wolfcoins, string user, Dictionary<int, Item> itemDatabase)
        {
            ...
            Logger.Info("{0} just looted a {1}!", user, newItem.itemName);
