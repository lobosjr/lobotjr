...
namespace Adventures
{
    ...
    class Dungeon
    {
        ...
        public Party RunDungeon(Party myParty, ref IrcClient whisper)
        {
            ...
            this.partySuccessRate *= percentPartyFull;
                ...
                if(!CalculateEncounterOutcome(encounterSuccess))
                {
                    ...
                    foreach (var player in myParty.members)
                    {
                        ...
                        if (CalculateDeath(partyMembers.ElementAt(partyIter), partyDeathAvoidance))
                        {
                            ...
                            int randomAmount = this.RNG.Next((randomRewardMod * -1), randomRewardMod);
                            xp += randomAmount;
        ...
        public void UpdateDungeonStats(float classAmount, CharClass player)
        {
            ...
            this.partySuccessRate += (updatedSuccess / classAmount);

            this.partyDeathAvoidance += (player.preventDeathBonus / classAmount);
            this.partyCoinBonus += (player.coinBonus / classAmount);
            this.partyItemFindBonus += (player.itemFind / classAmount);
            this.partyXpBonus += (player.xpBonus / classAmount);
        }
    }
}
